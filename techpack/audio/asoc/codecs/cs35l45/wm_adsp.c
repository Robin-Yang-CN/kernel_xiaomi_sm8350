		      ...
		      unsigned int alg, void *buf, size_t len)
{
	...
	if (!ctl) {
		adsp_err(dsp, "'%s'  name=%s  type=%d  alg=%x \n", ctl_name,name,type,alg);
		return -EINVAL;
	}
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	adsp_dbg(dsp, "put cal_z = %d, cal_checksum = %d\n", dsp->cal_z, dsp->cal_chksum);
